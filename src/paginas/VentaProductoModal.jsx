...
export default function VentaProductoModal({
  ...
  const confirmarVenta = async (e) => {
    ...
    if (supabase && opticaId) {
      ...
      if (errorRpc) {
        setError(
          errorRpc.message?.includes("stock")
            ? "No hay suficiente stock disponible para esta venta."
            : esErrorSinPermiso(errorRpc)
              ? MENSAJE_SIN_PERMISO
              : "No se pudo registrar la venta. Revisa tu conexión e intenta de nuevo."
        )
        setGuardando(false)
        return
      }
          ...
          <div className="min-h-0 flex-1 space-y-4 overflow-y-auto p-6">
            ...
            {agregandoProducto ? (
              ...
              <div>
                ...
                {productoFijo ? (
                  <div className="flex items-center justify-between rounded-xl border border-slate-200/60 bg-slate-50 px-3 py-2.5">
              ...
              <>
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    ...
                    <div className="rounded-xl border border-slate-200/60 bg-slate-50 px-3 py-2.5 text-right font-mono text-sm font-bold text-slate-800">${montoTotal.toFixed(2)}</div>
                  </div>
                </div>
